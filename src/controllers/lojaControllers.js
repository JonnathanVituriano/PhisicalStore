...
exports.buscaLojasProximas = async (req, res) => {
    ...
    try {
        ...
        const lojasProximas = lojas.filter(loja => {
            const distancia = calcularDistancia(enderecoUsuario.lat, enderecoUsuario.lng, loja.cordenadas.lat, loja.cordenadas.lng);
            return distancia <= 100;
        });

        if (lojasProximas.lenght === 0) {
            return res.status(404).json({ message: 'Nenhuma loja encontrada em um raio de 100km'});
        }

        lojasProximas.sort((a, b) => calcularDistancia(enderecoUsuario.lat, enderecoUsuario.lng, a.coordenadas.lat, a.cordenadas.lng) - calcularDistancia(enderecoUsuario.lat, enderecoUsuario.lng, b.coordenadas.lat, b.coordenadas.lng));
